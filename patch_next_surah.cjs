...
const regex = /function getNextSurah\(currentId: number\) \{[\s\S]*?return null;\n\}\s*\} else \{\s*if \(currentId === 45\) return null; \/\/ Finished everything\s*return SURAH_LIST\.find\(s => s\.id === currentId \+ 1\);\s*\}\s*\}/;
